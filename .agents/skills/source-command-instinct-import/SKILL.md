...
このコマンドは `continuous-learning-v2` スキルがインストールされていることを前提とする。インストールパスは `${CLAUDE_PLUGIN_ROOT}/skills/continuous-learning-v2/` または `~/.Codex/skills/continuous-learning-v2/`。Python 3 が必要。URL からインポートする場合は `curl` または `wget` が利用可能である必要がある。
...
新しいインスティンクトの保存先: ~/.Codex/homunculus/instincts/inherited/
